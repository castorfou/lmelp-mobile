...
Le mécanisme ADB ci-dessus (USB + laptop + build debug) devient impraticable avec la migration de `docker-lmelp` vers un NAS ([docker-lmelp#47](https://github.com/castorfou/docker-lmelp/issues/47)) — plus de laptop branché en USB en usage courant. `lmelp.db` est publié comme asset d'une **GitHub Release dédiée**, taguée `data-v{N}` où `N` est le `PRAGMA user_version`/`ROOM_VERSION` courant (repo `castorfou/lmelp-mobile`), avec un `metadata.json` à côté (date d'export, `sha256`, compteurs). L'app télécharge et remplace elle-même la base, sans dépendance ADB/USB/build debug.

⚠️ **Tag `data-v{N}` dynamique, pas `data-latest` fixe** (issue #132) : chaque app télécharge toujours le tag correspondant à son propre schéma Room local (`OkHttpGitHubReleaseApi` résout `data-v{N}` via `SqliteSchemaVersion.read()`, lecture directe du header binaire SQLite — octets 60-63, pas de dépendance `SQLiteDatabase`/JDBC). Le pipeline (`scripts/docker_export_and_publish_release.sh`) publie toujours vers le tag dérivé de `ROOM_VERSION` (via `export_mongo_to_sqlite.py --print-room-version`), sans conserver d'historique — un seul tag actif par schéma. Si une app reste sur un ancien schéma après que le pipeline soit passé au tag suivant, elle ne trouve simplement plus de release (404) et n'a pas de mise à jour de données proposée, plutôt que de télécharger un fichier incompatible qui déclencherait silencieusement `fallbackToDestructiveMigration()` (perte de toutes les données locales sans message d'erreur).

⚠️ **Bump de `ROOM_VERSION` = publier manuellement `data-v{N}` avant de pouvoir tester** : conséquence directe du point ci-dessus — tant que la release `data-v{N}` du nouveau schéma n'existe pas sur GitHub, une app buildée sur ce nouveau schéma ne trouve aucune donnée à télécharger (404, comportement voulu, mais bloque le test local). Procédure complète (régénération mini-DB, publication manuelle temporaire, piège de nommage d'asset déjà rencontré, comportement au merge) : [docs/dev/schema-migration-workflow.md](docs/dev/schema-migration-workflow.md). Rare — la grande majorité des évolutions de l'app ne touchent pas au schéma Room.
...
⚠️ **`db_metadata.version` n'avance que si le contenu a réellement changé** (issue #128) : l'anacron du NAS republie `lmelp.db` chaque jour inconditionnellement, mais `write_metadata()` compare désormais un `content_hash` (empreinte SHA-256 du contenu métier, calculé par `compute_content_hash()`) à celui de la dernière release GitHub publiée — récupéré par `docker_export_and_publish_release.sh` via `gh release download` avant l'export. Si le contenu est identique, `version` est réutilisée telle quelle (pas de nouveau timestamp) et la publication (`gh release upload`) est carrément sautée. Sans ça, l'app détectait une « mise à jour disponible » quasiment tous les matins même sans nouvelle donnée côté Masque et la Plume.
